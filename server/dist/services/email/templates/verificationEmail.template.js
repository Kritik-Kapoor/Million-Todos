...
export function buildVerificationEmailTemplate(email, username, verificationToken) {
    const safeUsername = escapeHtml(username);
    const verifyUrl = buildAppUrl(`/verify-email-token?token=${encodeURIComponent(verificationToken)}&email=${encodeURIComponent(email)}`);
